...
def cmd_manifest(a, graph: dict, schema: dict) -> int:
    ...
    def _as_paths(v) -> list[str]:
        """A comma-separated string renders one row per character, so coerce at the point of use.

        `record` already normalizes this, but a receipt that reached the log any other way — written
        by hand, or by a future path that forgets — still reaches this renderer, and the failure is
        silent and plausible-looking rather than loud. Normalizing where the value is consumed is
        what makes the bad rendering unreachable rather than merely unlikely.
        """
        if isinstance(v, str):
    ...
    for it in items:
        ...
        arts = _as_paths(rec.get("artifacts")) or _as_paths(it.get("artifacts")) or ["—"]
